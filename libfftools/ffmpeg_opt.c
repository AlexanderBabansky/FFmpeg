...
#define MATCH_PER_STREAM_OPT(name, type, outvar, fmtctx, st, returnValue)\
{\
    int i, ret, matches = 0;\
    SpecifierOpt *so;\
...
int print_stats       = -1;
int qp_hist           = 0;
int stdin_interaction = 0;  // disable stdin interaction
...
static int copy_metadata(char *outspec, char *inspec, AVFormatContext *oc, AVFormatContext *ic, OptionsContext *o)
{
    ...
    if(parse_meta_type(inspec,  &type_in,  &idx_in,  &istream_spec)){
        return -1;
    }
    if(parse_meta_type(outspec, &type_out, &idx_out, &ostream_spec)){
        return -1;
    }
...
static int opt_recording_timestamp(void *optctx, const char *opt, const char *arg)
{
    ...
    char buf[128];
    int ok;
    int64_t recording_timestamp = parse_time_or_die(opt, arg, 0,&ok) / 1E6;
    if(!ok){
        return -1;
    }
...
static AVCodec *find_codec(const char *name, enum AVMediaType type, int encoder)
{
...
static AVCodec *choose_decoder(OptionsContext *o, AVFormatContext *s, AVStream *st, int* ok)
{
    char *codec_name = NULL;

    MATCH_PER_STREAM_OPT(codec_names, str, codec_name, s, st, NULL);
    if (codec_name) {
        AVCodec *codec = find_codec(codec_name, st->codecpar->codec_type, 0);
        if(!codec){
            *ok=0;
            return NULL;
        }
...
static void add_input_streams(OptionsContext *o, AVFormatContext *ic, int* ok)
{
    int i, ret;
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        ist->dec = choose_decoder(o, ic, st, ok);
        if(!(*ok)){
            return;
        }
        ist->decoder_opts = filter_codec_opts(o->g->codec_opts, ist->st->codecpar->codec_id, ic, st, ist->dec, ok);
...
static int dump_attachment(AVStream *st, const char *filename)
{
    ...
    if(assert_file_overwrite(filename)){
        return -1;
    }
...
static int open_input_file(OptionsContext *o, const char *filename)
{
    ...
    if (video_codec_name){
        ic->video_codec    = find_codec(video_codec_name   , AVMEDIA_TYPE_VIDEO   , 0);
        if(!ic->video_codec){
            return -1;
        }
    }
    if (audio_codec_name){
        ic->audio_codec    = find_codec(audio_codec_name   , AVMEDIA_TYPE_AUDIO   , 0);
        if(!ic->audio_codec){
            return -1;
        }
    }
    if (subtitle_codec_name){
        ic->subtitle_codec = find_codec(subtitle_codec_name, AVMEDIA_TYPE_SUBTITLE, 0);
        if(!ic->subtitle_codec){
            return -1;
        }
    }
    if (data_codec_name){
        ic->data_codec     = find_codec(data_codec_name    , AVMEDIA_TYPE_DATA    , 0);
        if(!ic->data_codec){
            return -1;
        }
    }
    ...
    if(assert_avoptions(o->g->format_opts)){
        return -1;
    }
    ...
    for (i = 0; i < ic->nb_streams; i++){
        int ok;
        choose_decoder(o, ic, ic->streams[i], &ok);
        if(!ok){
            return -1;
        }
    }
    ...
    if (find_stream_info) {
        int ok;
        AVDictionary **opts = setup_find_stream_info_opts(ic, o->g->codec_opts, &ok);
        if(!ok){
            return -1;
        }
    ...
    for (i = 0; i < o->nb_dump_attachment; i++) {
        ...
        for (j = 0; j < ic->nb_streams; j++) {
            ...
            if (check_stream_specifier(ic, st, o->dump_attachment[i].specifier) == 1){
                if(dump_attachment(st, o->dump_attachment[i].u.str)){
                    return -1;
                }
            }
        }
...
static int choose_encoder(OptionsContext *o, AVFormatContext *s, OutputStream *ost)
{
    ...
    if (type == AVMEDIA_TYPE_VIDEO || type == AVMEDIA_TYPE_AUDIO || type == AVMEDIA_TYPE_SUBTITLE) {
        int oktmp;
        int* ok=&oktmp;
        MATCH_PER_STREAM_OPT(codec_names, str, codec_name, s, ost->st, -1);
        if (!codec_name) {
            ...
            ost->enc = find_codec(codec_name, ost->st->codecpar->codec_type, 1);
            if(!ost->enc){
                return -1;
            }
...
static OutputStream *new_output_stream(OptionsContext *o, AVFormatContext *oc, enum AVMediaType type, int source_index, int* ok)
{
    ...
    if (ost->enc) {
        ...
        ost->encoder_opts  = filter_codec_opts(o->g->codec_opts, ost->enc->id, oc, st, ost->enc, ok);
        if(!(*ok)){
            return ost;
        }

        MATCH_PER_STREAM_OPT(presets, str, preset, oc, st, ost);
        ...
        ost->encoder_opts = filter_codec_opts(o->g->codec_opts, AV_CODEC_ID_NONE, oc, st, NULL, ok);
        if(!(*ok)){
            return ost;
        }
    }
    ...
    MATCH_PER_STREAM_OPT(time_bases, str, time_base, oc, st, ost);
    if (time_base) {
        AVRational q;
    ...
    MATCH_PER_STREAM_OPT(enc_time_bases, str, time_base, oc, st, ost);
    if (time_base) {
        AVRational q;
        if (av_parse_ratio(&q, time_base, INT_MAX, 0, NULL) < 0 ||
            q.den <= 0) {
    ...
    MATCH_PER_STREAM_OPT(max_frames, i64, ost->max_frames, oc, st, ost);
    ...
    MATCH_PER_STREAM_OPT(codec_tags, str, codec_tag, oc, st, ost);
    if (codec_tag) {
        uint32_t tag = strtol(codec_tag, &next, 0);
        if (*next)
            tag = AV_RL32(codec_tag);
    ...
    MATCH_PER_STREAM_OPT(qscale, dbl, qscale, oc, st, ost);
    if (qscale >= 0) {
        ost->enc_ctx->flags |= AV_CODEC_FLAG_QSCALE;
        ost->enc_ctx->global_quality = FF_QP2LAMBDA * qscale;
    }

    MATCH_PER_STREAM_OPT(disposition, str, ost->disposition, oc, st, ost);
    ost->disposition = av_strdup(ost->disposition);

    ost->max_muxing_queue_size = 128;
    MATCH_PER_STREAM_OPT(max_muxing_queue_size, i, ost->max_muxing_queue_size, oc, st, ost);
...
static void parse_matrix_coeffs(uint16_t *dest, const char *str, int* ok)
{
    int i;
    const char *p = str;
    for (i = 0;; i++) {
...
static OutputStream *new_video_stream(OptionsContext *o, AVFormatContext *oc, int source_index, int* ok)
{
    AVStream *st;
    OutputStream *ost;
    ...
    ost = new_output_stream(o, oc, AVMEDIA_TYPE_VIDEO, source_index, ok);
    if(!(*ok)){
        return ost;
    }
    st  = ost->st;
    ...
    MATCH_PER_STREAM_OPT(frame_rates, str, frame_rate, oc, st, ost);
    ...
    MATCH_PER_STREAM_OPT(frame_aspect_ratios, str, frame_aspect_ratio, oc, st, ost);
    ...
    MATCH_PER_STREAM_OPT(filter_scripts, str, ost->filters_script, oc, st, ost);
    MATCH_PER_STREAM_OPT(filters,        str, ost->filters,        oc, st, ost);
    ...
    if (!ost->stream_copy) {
        ...
        MATCH_PER_STREAM_OPT(frame_sizes, str, frame_size, oc, st, ost);
        ...
        MATCH_PER_STREAM_OPT(frame_pix_fmts, str, frame_pix_fmt, oc, st, ost);
        ...
        MATCH_PER_STREAM_OPT(intra_matrices, str, intra_matrix, oc, st, ost);
        if (intra_matrix) {
            ...
            parse_matrix_coeffs(video_enc->intra_matrix, intra_matrix,ok);
            if(!(*ok)){
                return ost;
            }
        }
        MATCH_PER_STREAM_OPT(chroma_intra_matrices, str, chroma_intra_matrix, oc, st, ost);
        if (chroma_intra_matrix) {
            ...
            parse_matrix_coeffs(p, chroma_intra_matrix, ok);
            if(!(*ok)){
                return ost;
            }
        }
        MATCH_PER_STREAM_OPT(inter_matrices, str, inter_matrix, oc, st, ost);
        if (inter_matrix) {
            ...
            parse_matrix_coeffs(video_enc->inter_matrix, inter_matrix, ok);
            if(!(*ok)){
                return ost;
            }
        }

        MATCH_PER_STREAM_OPT(rc_overrides, str, p, oc, st, ost);
        for (i = 0; p; i++) {
            int start, end, q;
        ...
        /* two pass mode */
        MATCH_PER_STREAM_OPT(pass, i, do_pass, oc, st, ost);
        if (do_pass) {
            if (do_pass & 1) {
        ...
        MATCH_PER_STREAM_OPT(passlogfiles, str, ost->logfile_prefix, oc, st, ost);
        ...
        MATCH_PER_STREAM_OPT(forced_key_frames, str, ost->forced_keyframes, oc, st, ost);
        if (ost->forced_keyframes)
            ost->forced_keyframes = av_strdup(ost->forced_keyframes);

        MATCH_PER_STREAM_OPT(force_fps, i, ost->force_fps, oc, st, ost);

        ost->top_field_first = -1;
        MATCH_PER_STREAM_OPT(top_field_first, i, ost->top_field_first, oc, st, ost);
        ...
        MATCH_PER_STREAM_OPT(copy_initial_nonkeyframes, i, ost->copy_initial_nonkeyframes, oc ,st, ost);
    }

    if (ost->stream_copy){
        if(check_streamcopy_filters(o, oc, ost, AVMEDIA_TYPE_VIDEO)){
            *ok=0;
            return ost;
        }
    }
...
static OutputStream *new_audio_stream(OptionsContext *o, AVFormatContext *oc, int source_index, int* ok)
{
    int n;
    AVStream *st;
    OutputStream *ost;
    ...
    ost = new_output_stream(o, oc, AVMEDIA_TYPE_AUDIO, source_index, ok);
    if(!(*ok)){
        return ost;
    }
    st  = ost->st;
    ...
    if (!ost->stream_copy) {
        ...
        MATCH_PER_STREAM_OPT(audio_channels, i, audio_enc->channels, oc, st, ost);

        MATCH_PER_STREAM_OPT(sample_fmts, str, sample_fmt, oc, st, ost);
        if (sample_fmt &&
        ...
        MATCH_PER_STREAM_OPT(audio_sample_rate, i, audio_enc->sample_rate, oc, st, ost);

        MATCH_PER_STREAM_OPT(apad, str, ost->apad, oc, st, ost);
    ...
    if (ost->stream_copy){
        if(check_streamcopy_filters(o, oc, ost, AVMEDIA_TYPE_AUDIO)){
            *ok=0;
            return ost;
        }
    }
...
static OutputStream *new_data_stream(OptionsContext *o, AVFormatContext *oc, int source_index, int* ok)
{
    OutputStream *ost;

    ost = new_output_stream(o, oc, AVMEDIA_TYPE_DATA, source_index, ok);
    if(!(*ok)){
        return ost;
    }
    if (!ost->stream_copy) {
...
static OutputStream *new_unknown_stream(OptionsContext *o, AVFormatContext *oc, int source_index, int* ok)
{
    OutputStream *ost;

    ost = new_output_stream(o, oc, AVMEDIA_TYPE_UNKNOWN, source_index, ok);
    if(!(*ok)){
        return ost;
    }
    if (!ost->stream_copy) {
...
static OutputStream *new_attachment_stream(OptionsContext *o, AVFormatContext *oc, int source_index, int* ok)
{
    OutputStream *ost = new_output_stream(o, oc, AVMEDIA_TYPE_ATTACHMENT, source_index, ok);
    if(!(*ok)){
        return ost;
    }
    ost->stream_copy = 1;
...
static OutputStream *new_subtitle_stream(OptionsContext *o, AVFormatContext *oc, int source_index, int* ok)
{
    AVStream *st;
    OutputStream *ost;
    AVCodecContext *subtitle_enc;

    ost = new_output_stream(o, oc, AVMEDIA_TYPE_SUBTITLE, source_index, ok);
    if(!(*ok)){
        return ost;
    }
    st  = ost->st;
...
static int opt_streamid(void *optctx, const char *opt, const char *arg)
{
    ...
    *p++ = '\0';
    int ok;
    idx = parse_number(opt, idx_str, OPT_INT, 0, MAX_STREAMS-1, &ok);
    if(!ok){
        return -1;
    }
    ...
    o->streamid_map[idx] = parse_number(opt, p, OPT_INT, 0, INT_MAX, &ok);
    if(!ok){
        return -1;
    }
    return 0;
                               ...
                               AVFormatContext *oc)
{
    ...
    int ok=1;
    switch (ofilter->type) {
    case AVMEDIA_TYPE_VIDEO: ost = new_video_stream(o, oc, -1, &ok); break;
    case AVMEDIA_TYPE_AUDIO: ost = new_audio_stream(o, oc, -1, &ok); break;
    default:
...
static int open_output_file(OptionsContext *o, const char *filename)
{
    ...
    for (i = 0; i < nb_filtergraphs; i++) {
        ...
        for (j = 0; j < fg->nb_outputs; j++) {
            ...
            if(init_output_filter(ofilter, o, oc)){
                return -1;
            }
        }
    }
...
loop_end:
                ...
                case AVMEDIA_TYPE_VIDEO:      ost = new_video_stream     (o, oc, src_idx, &ok); break;
                case AVMEDIA_TYPE_AUDIO:      ost = new_audio_stream     (o, oc, src_idx, &ok); break;
                case AVMEDIA_TYPE_SUBTITLE:   ost = new_subtitle_stream  (o, oc, src_idx, &ok); break;
                case AVMEDIA_TYPE_DATA:       ost = new_data_stream      (o, oc, src_idx, &ok); break;
                case AVMEDIA_TYPE_ATTACHMENT: ost = new_attachment_stream(o, oc, src_idx, &ok); break;
                case AVMEDIA_TYPE_UNKNOWN:
                    if (copy_unknown_streams) {
                        ost = new_unknown_stream   (o, oc, src_idx, &ok);
                        break;
                    }
                default:
    ...
    for (i = 0; i < o->nb_attachments; i++) {
        ...
        int ok;
        ost = new_attachment_stream(o, oc, -1, &ok);
        if(!ok){
            return -1;
        }
        ost->stream_copy               = 0;
    ...
    for (i = 0; i < o->nb_metadata; i++) {
        ...
        *val++ = 0;
        ...
        if(parse_meta_type(o->metadata[i].specifier, &type, &index, &stream_spec)){
            return -1;
        }
        if (type == 's') {
...
static int opt_vsync(void *optctx, const char *opt, const char *arg)
{
    ...
    if (video_sync_method == VSYNC_AUTO){
        int ok;
        video_sync_method = parse_number("vsync", arg, OPT_INT, VSYNC_AUTO, VSYNC_VFR,&ok);
        if(!ok){
            return -1;
        }
    }
...
void show_usage(void)
{
    ...
    av_log(NULL, AV_LOG_INFO, "usage: ffmpeg [options] [[infile options] -i infile]... {[outfile options] outfile}...\n");
